...
# ── Phone OTP store (in-memory, testing only) ──────────────────
# Format: { phone: { otp: "1234", expires: timestamp, user_id: id } }
_phone_otp_store = {}

@app.route("/send-otp", methods=["POST"])
@limiter.limit("5 per minute")
def send_otp():
    """Send (fake) OTP to phone number for login."""
    ...
    # Fixed OTP for testing — replace with real SMS API later
    otp = "1234"
    _phone_otp_store[phone] = {
        "otp": otp,
        "expires": _time.time() + 300,  # 5 min expiry
        "user_id": user["id"]
    }

    # In production: send SMS here
    # requests.post("https://sms-api.com/send", json={"to": phone, "msg": f"Your OTP is {otp}"})

    flash(f"OTP bhej diya! (Testing mode: OTP = 1234)", "ok")
    session["otp_phone"] = phone  # remember which phone is being verified
...
def verify_otp():
    ...
    if request.method == "POST":
        ...
        if _time.time() > stored["expires"]:
            ...
            flash("OTP expire ho gaya (5 min). Dobara try karo.", "err")
            return redirect("/login")
...
def login():
    """Login page — sirf phone number se, OTP ke zariye."""
    if session.get("user_id"):
        return redirect("/")
    if request.method == "POST":
        check_csrf()
        ...
        if user["seller_status"] in ("suspended", "deleted"):
            flash("Aapka account suspend hai. Support se contact karo.", "err")
            return redirect("/login")
        # Redirect to send-otp with phone prefilled
...
def send_otp_go():
    """Internal redirect — sends OTP for phone stored in session."""
    import time as _time
    phone = session.get("otp_phone", "")
    if not phone:
        return redirect("/login")
    ...
    if not user:
        return redirect("/login")
    otp = "1234"  # Testing mode — replace with real SMS API later
    _phone_otp_store[phone] = {
        "otp": otp,
