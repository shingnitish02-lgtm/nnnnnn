...
def init_db():
    ...
    user_extra_cols = [('rzp_contact_id', 'TEXT'), ('rzp_fund_account_id', 'TEXT')]
...
# ─────────────────────────────────────────────────────────────
# AUTO-SETTLEMENT ENGINE  (Meesho-style 7-day cycle)
# ─────────────────────────────────────────────────────────────

SETTLEMENT_DAYS = int(os.getenv("SETTLEMENT_DAYS", "7"))   # default 7 din baad delivery
...
def bake_shipping_into_price(rows, buyer_state=None):
    """
    State-wise shipping charge product price mein add karo:
    - Same state (seller == buyer): ₹75
    - Different state: ₹95
    - Buyer state unknown / not logged in: ₹85 (default)
    Seller ka state product ke saath aana chahiye as 'seller_state'.
    """
    if rows is None:
        return rows
        ...
        try:
            ...
            if buyer_state and seller_state:
                if buyer_state.strip().lower() == seller_state.strip().lower():
                    ship = SHIPPING_SAME_STATE
                else:
                    ship = SHIPPING_OTHER_STATE
            else:
                ship = SHIPPING_DEFAULT
...
def cart_summary(user_id):
    ...
    for r in all_rows:
        ...
        base_price = r["price"]
        ship       = r.get("shipping_charge") or 0
...
# ── AUTH ROUTES
# ─────────────────────────────────────────────────────────────
# In-memory brute force tracker: {ip: [timestamp1, timestamp2, ...]}
_login_attempts = {}

# ── Phone OTP store (in-memory) ──────────────────────────────
# Format: { phone: { otp: "123456", expires: timestamp, user_id: id } }
_phone_otp_store = {}

WHATSAPP_BOT_URL = os.getenv("WHATSAPP_BOT_URL", "http://localhost:3000")

def send_whatsapp_otp(phone, otp):
    """WhatsApp bot ko call karke OTP bhejo."""
    import requests as _req
    try:
        resp = _req.post(
...
def send_otp():
    ...
    conn = get_db()
    user = _exec(conn, "SELECT * FROM users WHERE phone=%s", (phone,)).fetchone()
    if not user:
    ...
    _phone_otp_store[phone] = {
        "otp": otp,
        "expires": _time.time() + 120,  # 2 min expiry
        "user_id": user["id"]
    }
...
def register():
    ...
    if request.method == "POST":
        ...
        if not re.match(r"^[6-9]\d{9}$", phone):
            ...
            return redirect("/register")
        conn = get_db()
        if _exec(conn, "SELECT id FROM users WHERE phone=%s", (phone,)).fetchone():
            flash("Yeh phone number pehle se registered hai. Login karo.", "err")
            return redirect("/login")
        my_ref = uuid.uuid4().hex[:8].upper()
        # email aur password blank — phone-only system
        # email blank rakho (NOT NULL ke liye empty string)
        fake_email = f"phone_{phone}@xoptime.local"
        _exec(conn, 
...
def index():
    ...
    featured = _exec(conn,
        "SELECT p.*, u.state as seller_state, COALESCE(agg.avg_rating,0) avg_rating, COALESCE(agg.review_count,0) review_count "
        "FROM products p JOIN users u ON p.seller_id=u.id "
        "LEFT JOIN (SELECT product_id, AVG(rating) avg_rating, COUNT(id) review_count "
        "FROM reviews GROUP BY product_id) agg ON agg.product_id=p.id "
    ...
    bake_shipping_into_price(products, buyer_state)
    bake_shipping_into_price(featured, buyer_state)
    bake_shipping_into_price(trending, buyer_state)
    bake_shipping_into_price(flash_sale_products, buyer_state)
...
def search():
    ...
    REV_AGG = ("LEFT JOIN (SELECT product_id, AVG(rating) avg_rating, COUNT(id) review_count "
               "FROM reviews GROUP BY product_id) agg ON agg.product_id=p.id ")
    sql = ("SELECT p.*, u.name as seller_name, u.state as seller_state, "
           "COALESCE(agg.avg_rating,0) avg_rating, COALESCE(agg.review_count,0) review_count "
    ...
    bake_shipping_into_price(products, get_buyer_state())
    categories  = [r["category"] for r in _exec(conn, 
        "SELECT DISTINCT category FROM products WHERE approved=1 ORDER BY category").fetchall()]
...
def product_detail(pid):
    ...
    if not p:
        abort(404)
    bake_shipping_into_price(p, buyer_state)
    images   = _exec(conn, 
    ...
    bake_shipping_into_price(similar, buyer_state)
    wished = False
    if session.get("role") == "buyer":
...
def checkout():
    ...
    if request.method == "POST":
        ...
        order_id = _scalar(_exec(conn, 
            """INSERT INTO orders (public_id,buyer_id,buyer_name,phone,address,pincode,city,state,
...
def wishlist():
    ...
    bake_shipping_into_price(items, get_buyer_state())
...
def seller_wallet():
    ...
    total_earned = _scalar(_exec(conn,
        "SELECT COALESCE(SUM(net_amount),0) FROM seller_transactions WHERE seller_id=%s AND status IN ('earned','settlement_due','paid')", (sid,)
    ))
    ...
    locked_amount = _scalar(_exec(conn,
        "SELECT COALESCE(SUM(net_amount),0) FROM seller_transactions WHERE seller_id=%s AND status='earned'", (sid,)
    ))
    due_amount = _scalar(_exec(conn,
        "SELECT COALESCE(SUM(net_amount),0) FROM seller_transactions WHERE seller_id=%s AND status='settlement_due'", (sid,)
    ))
    total_commission = _scalar(_exec(conn,
        "SELECT COALESCE(SUM(commission),0) FROM seller_transactions WHERE seller_id=%s", (sid,)
    ))

    # Next payout date — earliest settlement_due_at
    next_payout_row = _exec(conn,
        "SELECT MIN(settlement_due_at) AS next_date FROM seller_transactions WHERE seller_id=%s AND status='earned'",
        (sid,)
    ).fetchone()
    next_payout_date = next_payout_row["next_date"] if next_payout_row else None
    ...
    wallet_stats = {
        ...
        "pending_payout":   round(locked_amount + due_amount, 2),
...
def seller_orders():
    ...
    params = [sid]
    if status == 'pending':
        # pending tab = placed + pending + confirmed sab
        sql += "AND o.status IN ('placed','pending','confirmed') "
    elif status:
        sql += "AND o.status=%s "
...
def seller_order_accept(oid):
    ...
    if not items:
        abort(403)
    o = _exec(conn, "SELECT * FROM orders WHERE id=%s", (oid,)).fetchone()
    if o["status"] not in ("pending", "confirmed", "placed"):
...
@app.route("/seller/orders/bulk-accept", methods=["POST"])
@seller_required
def seller_orders_bulk_accept():
    check_csrf()
    conn = get_db()
    sid  = session["user_id"]
    order_ids = request.form.getlist("order_ids[]")
    if not order_ids:
        flash("Koi order select nahi kiya.", "err")
        return redirect("/seller/orders?status=pending")
    ...
    for oid in order_ids:
        ...
        if not items:
            continue
        o = _exec(conn, "SELECT * FROM orders WHERE id=%s", (oid,)).fetchone()
        if not o or o["status"] not in ("pending", "confirmed", "placed"):
            continue
        _exec(conn, "UPDATE orders SET status='accepted', updated_at=NOW() WHERE id=%s", (oid,))
        add_notification(o["buyer_id"], "Order Accepted",
                         f"Your order #{o['public_id']} has been accepted by the seller!", "/orders")
        accepted += 1

    conn.commit()
    flash(f"{accepted} order(s) accept kar liye!", "ok")
    return redirect("/seller/orders?status=pending")
...
def seller_order_ready(oid):
    ...
    if courier_result["success"]:
        ...
        shipment_id  = courier_result.get("shipment_id", "")
        tracking_url = f"{tracking_base}{awb}" if awb else ""

        notes_val = f"Shiprocket shipment_id: {shipment_id}"
...
def seller_label(oid):
    ...
    try:
        ...
        order_qr_b64 = base64.b64encode(buf.getvalue()).decode()

        awb_val = o["awb"] or o["tracking_id"]
        if awb_val:
            awb_qr  = qrcode.make(awb_val)
            awb_buf = _io.BytesIO()
            awb_qr.save(awb_buf, format="PNG")
            awb_qr_b64 = base64.b64encode(awb_buf.getvalue()).decode()
    except ImportError:
        pass
    return render_template("label.html", o=o, items=items, seller=seller_obj,
                           company_name=COMPANY_NAME, order_qr_b64=order_qr_b64, awb_qr_b64=awb_qr_b64)
...
def seller_product_form(pid=None):
    ...
    if request.method == "POST":
        ...
        extra_vals = (generic_name, material, pattern, occasion, country_of_origin,
                      ...
                      size_chart_data, shipping_charge)
        ...
        if pid:
            ...
            _exec(conn, 
                """INSERT INTO products (seller_id,title,category,description,brand,price,mrp,
                   ...
                   packer_name,packer_address,packer_pincode,
                   importer_name,importer_address,importer_pincode,tags,
                   closure,fold_type,product_height,product_type,compartments,size_chart_data,shipping_charge)
                   VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,0,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
...
def seller_profile():
    ...
    if request.method == "POST":
        ...
        # Address change hone pe Shiprocket pickup re-register karo
        if address and seller.get("seller_status") == "active":
            updated_seller = _exec(conn, "SELECT * FROM users WHERE id=%s", (session["user_id"],)).fetchone()
            pickup_name = shiprocket_register_pickup_address(updated_seller)
            if pickup_name:
                _exec(conn, "UPDATE users SET shiprocket_pickup_name=%s WHERE id=%s",
                      (pickup_name, session["user_id"]))
                conn.commit()
...
def admin_order_detail(oid):
    ...
    if request.method == "POST":
        ...
        if status == "delivered" and not delivered_at:
            ...
            # Mark seller transactions as earned + settlement due date set karo
            _exec(conn,
                """UPDATE seller_transactions
                   SET status='earned',
                       settlement_due_at = NOW() + INTERVAL '""" + str(SETTLEMENT_DAYS) + """ days'
                   WHERE order_id=%s AND status='pending'""", (oid,)
            )
        _exec(conn, 
...
def admin_label(oid):
    ...
    if items and items[0]["seller_id"]:
        seller_obj = _exec(conn, "SELECT * FROM users WHERE id=%s", (items[0]["seller_id"],)).fetchone()
    # Generate separate QR codes — one for order public_id, one for AWB
    order_qr_b64 = None
    awb_qr_b64   = None
    try:
...
@app.route("/admin/sellers/<int:uid>/register-pickup", methods=["POST"])
@admin_required
def admin_register_seller_pickup(uid):
    """Admin se manually seller ka Shiprocket pickup register karo."""
    check_csrf()
    conn = get_db()
    seller = _exec(conn, "SELECT * FROM users WHERE id=%s", (uid,)).fetchone()
    if not seller:
        flash("Seller nahi mila.", "err")
        return redirect("/admin/sellers")
    pickup_name = shiprocket_register_pickup_address(seller)
    if pickup_name:
        _exec(conn, "UPDATE users SET shiprocket_pickup_name=%s WHERE id=%s", (pickup_name, uid))
        conn.commit()
        flash(f"Shiprocket pickup registered: {pickup_name}", "ok")
    else:
        flash("Pickup register nahi hua. Seller ka address/phone check karo.", "err")
    return redirect("/admin/sellers")
...
def admin_seller_approve(uid):
    ...
    # Shiprocket pe seller ka pickup address auto-register karo
    seller = _exec(conn, "SELECT * FROM users WHERE id=%s", (uid,)).fetchone()
    if seller and not seller.get("shiprocket_pickup_name"):
        pickup_name = shiprocket_register_pickup_address(seller)
        if pickup_name:
            _exec(conn, "UPDATE users SET shiprocket_pickup_name=%s WHERE id=%s", (pickup_name, uid))
            conn.commit()
            flash(f"Seller approved. Shiprocket pickup registered: {pickup_name}", "ok")
        else:
            flash("Seller approved. Shiprocket pickup register nahi hua — seller ka address check karo.", "warn")
    else:
...
def admin_payouts():
    ...
    sql = """
        SELECT st.*, u.name AS seller_name, u.bank_account, u.bank_ifsc,
               u.bank_name, u.upi_id, u.email, u.phone,
               u.rzp_contact_id, u.rzp_fund_account_id,
               o.public_id
        FROM seller_transactions st
    ...
    summary = _exec(conn, """
        SELECT
            COALESCE(SUM(CASE WHEN status='earned' THEN net_amount END),0)          AS total_earned,
            COALESCE(SUM(CASE WHEN status='settlement_due' THEN net_amount END),0)  AS total_due,
            COALESCE(SUM(CASE WHEN status='paid' THEN net_amount END),0)            AS total_paid,
            COUNT(CASE WHEN status='settlement_due' THEN 1 END)                     AS count_due,
            COALESCE(SUM(commission),0)                                             AS total_commission
        FROM seller_transactions
    """).fetchone()

    return render_template("admin_payouts.html", txns=txns, summary=summary,
...
def admin_payout_mark_paid(tid):
    ...
    _exec(conn,
          "UPDATE seller_transactions SET status='paid', settled_at=NOW(), utr=%s, payout_mode='manual' WHERE id=%s",
          (utr or None, tid))
    conn.commit()
    flash("✅ Payment marked as paid.", "ok")
    return redirect("/admin/payouts?status=settlement_due")


@app.route("/admin/payouts/razorpay-pay/<int:tid>", methods=["POST"])
@admin_required
def admin_payout_razorpay(tid):
    """Single transaction Razorpay se auto-pay."""
    check_csrf()
    if not AUTO_PAYOUT_ENABLED:
        flash("⚠️ Auto-payout filhal disabled hai (Razorpay KYC pending). Manual se mark karo.", "err")
    ...
    """, (tid,)).fetchone()
    if not txn:
        flash("Transaction nahi mila.", "err")
        return redirect("/admin/payouts")
    if not RAZORPAY_AVAILABLE or not RZP_KEY_ID:
        flash("Razorpay configured nahi hai.", "err")
        return redirect("/admin/payouts")
    if not os.getenv("RAZORPAY_ACCOUNT_NUMBER"):
        flash("⚠️ RAZORPAY_ACCOUNT_NUMBER .env mein set karo (Razorpay dashboard → Account & Settings → Bank Account).", "err")
        return redirect("/admin/payouts")
    try:
        success, result = _rzp_payout(conn, txn, txn)
        if success:
            flash(f"✅ Payout initiated! Razorpay ID: {result}", "ok")
        else:
            flash(f"❌ Payout failed: {result}", "err")
    except Exception as e:
        flash(f"❌ Error: {e}", "err")
    return redirect("/admin/payouts?status=settlement_due")


@app.route("/admin/payouts/batch-pay", methods=["POST"])
@admin_required
def admin_payout_batch():
    """Saare settlement_due transactions ek saath process karo."""
    check_csrf()
    conn = get_db()
    use_razorpay = (
        AUTO_PAYOUT_ENABLED
        and RAZORPAY_AVAILABLE and RZP_KEY_ID
        and os.getenv("RAZORPAY_ACCOUNT_NUMBER", "")
        and "live" in RZP_KEY_ID
    )
    due_txns = _exec(conn, """
        SELECT st.*, u.name AS seller_name, u.bank_account, u.bank_ifsc,
               u.bank_name, u.upi_id, u.email, u.phone,
               u.rzp_contact_id, u.rzp_fund_account_id
        FROM seller_transactions st JOIN users u ON st.seller_id=u.id
        WHERE st.status='settlement_due'
    """).fetchall()
    success_count, fail_count = 0, 0
    for txn in due_txns:
        try:
            if use_razorpay and (txn["bank_account"] or txn["upi_id"]):
                ok, _ = _rzp_payout(conn, txn, txn)
                if ok: success_count += 1
                else:   fail_count += 1
            else:
                _exec(conn,
                    "UPDATE seller_transactions SET status='paid', settled_at=NOW(), payout_mode='manual' WHERE id=%s",
                    (txn["id"],))
                conn.commit()
                success_count += 1
        except Exception as e:
            fail_count += 1
            app.logger.error(f"Batch payout error txn#{txn['id']}: {e}")
    flash(f"{'✅' if fail_count==0 else '⚠️'} Batch done: {success_count} paid, {fail_count} failed.",
          "ok" if fail_count == 0 else "err")
    return redirect("/admin/payouts?status=settlement_due")


@app.route("/admin/payouts/run-settlement", methods=["POST"])
@admin_required
def admin_run_settlement():
    """Manually settlement cycle trigger karo."""
    check_csrf()
    import threading as _th
    _th.Thread(target=run_settlement_cycle, daemon=True).start()
    flash("⚡ Settlement cycle triggered! Thodi der mein process hoga.", "ok")
    return redirect("/admin/payouts")
...
# ── Auto Settlement: Render Cron Job HTTP endpoint ──────────
# Background thread nahi — Render cron /cron/run-settlement call karega har 6 ghante
# CRON_SECRET env var se secure hai

@app.route("/cron/run-settlement", methods=["POST"])
def cron_run_settlement():
    """Render cron job endpoint — settlement cycle trigger karta hai."""
    ...
    try:
        ...
        return {"status": "ok", "msg": "Settlement cycle complete"}, 200
    except Exception as e:
        app.logger.error(f"Cron settlement error: {e}")
